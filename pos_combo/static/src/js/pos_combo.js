...
odoo.define('pos_combo.pos', function(require) {
	"use strict";
	...
	models.Orderline = models.Orderline.extend({
		...
		set_combo_price: function(price){
			var prods = this.get_combo_products()
			var total = 0;
			var required_products = self.pos.get('required_products')
			prods.forEach(function (prod) {
				if(prod){
					total += prod.lst_price
				}
			});
			required_products.forEach(function (required_product) {
				if (required_product) {
					total -= required_product.lst_price
				}
			});
